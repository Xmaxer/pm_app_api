  ...
  module ApiKeysMutations
    class ApiKeyMutation < BaseMutationAuthenticable
      ...
      def resolve(**args)
        ...
        api_key = company.api_keys.find_or_initialize_by(id: args[:api_key_details][:id])
        attrs = args[:api_key_details].to_h.except(:id)
        if api_key.new_record?
          attrs[:user_id] = user.id
          api_key = ApiKey.new(attrs)
        else
          api_key.attributes = attrs
        end

        Exceptions::ExceptionHandler.to_graphql_execution_error_array(api_key.errors).each { |error| context.add_error(error) } unless api_key.valid? && api_key.save
