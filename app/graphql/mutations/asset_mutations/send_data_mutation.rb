module Mutations
  module AssetMutations
    class SendDataMutation < BaseMutationAuthenticable
      ...
      argument :separator, String, required: true, description: "CSV data separator"
      ...
      def resolve(**args)
        ...
        args[:data].each do |d|
          size = d.split(args[:separator]).size
        ...
        args[:data].each do |d|
          ...
          %x(ruby #{Rails.root.to_s}/grpc/send_data.rb "#{args[:asset_id]}" "#{d}" "#{args[:separator]}")
        end
